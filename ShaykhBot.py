...
def get_ayah(intent_request):
    ...
    surah_number = get_slots(intent_request)["Surah"]
    source = intent_request['invocationSource']
    if int(surah_number) < 0 or  int(surah_number) >114 : 
       #Incorrect value passed. 
        logger.debug("Value passed for surah number..{}".format(surah_number))
        error_msg =  { \
        "contentType": "PlainText", \
        "content": "Chapter Number should be between 1 to 114" \
        }
    ...
    url = 'http://api.alquran.cloud/ayah/'+surah_number+':'+ayah_number+'/editions/en.sahih'
    logger.debug("request url..{}".format(url))
    response = requests.get(url)
    resp_data = response.json()
    if resp_data['status'][0] in "Bad Request" : 
        resp_text = resp_data['data'][0]
    else : 

         resp_dict = dict(resp_data['data'][0])
         resp_text = resp_dict['text']
    
    logger.debug(resp_data['data'])
